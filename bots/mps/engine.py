...
piece_value = {                            #defines value for each piece
    chess.PAWN: 100,
    chess.ROOK: 500,
    chess.KNIGHT: 320,
    chess.BISHOP: 330,
...
def move_value(board: chess.Board, move: chess.Move, endgame: bool) -> float:
    ...
    return current_move_value
                                               #these functions are used for evaluation: move_value evaluate_capture evaluate_piece evaluate_board check_end_game  
...
def evaluate_board(board: chess.Board) -> float:      #evaluates board
    total = 0
...
def minimax_root(depth: int, board: chess.Board) -> chess.Move:     #minimax algo
...
class ChessAI:
    ...
    def make_move(self, board, debug=True) -> str:
        ...
        depth = 3
        move = str(minimax_root(depth, board))         #initiates the minimax algo
    ...
    def add_move(self, board, move: str) -> None:
